...
export default function Menu({ dict }: { dict: MenuDict }) {
  const [activeKey, setActiveKey] = useState(dict.categories[0]?.key ?? "");

  const activeCategory = dict.categories.find((c) => c.key === activeKey) ?? dict.categories[0];
  ...
  return (
    <section id="menu" className="px-6 py-16 md:px-8 md:py-24 xl:px-16 xl:py-32">
      <div className="mx-auto max-w-5xl">
        ...
        >
          <h2 className="text-4xl font-black tracking-tight sm:text-5xl">{dict.title}</h2>
          <p className="mt-4 text-lg text-zinc-500">{dict.subtitle}</p>
        </motion.div>
